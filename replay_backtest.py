...
from monitor import _entry_signal_score, _impulse_speed_entry_guard, _signal_priority
...
def _entry_candidate(feat: dict, i: int, c: np.ndarray, tf: str) -> Optional[Tuple[str, float, int]]:
    ...
    if brk_ok:
        return "breakout", getattr(config, "ATR_TRAIL_K_BREAKOUT", 1.5), getattr(config, "MAX_HOLD_BARS_BREAKOUT", 6)
    if ret_ok:
        return "retest", getattr(config, "ATR_TRAIL_K_RETEST", 1.8), getattr(config, "MAX_HOLD_BARS_RETEST", 10)
    if entry_ok:
        ...
        return mode, trail_k, max_hold
    if surge_ok:
        return "impulse_speed", getattr(config, "ATR_TRAIL_K_STRONG", 2.5), getattr(config, "MAX_HOLD_BARS_15M", 48)
    if imp_ok:
        return "impulse", getattr(config, "ATR_TRAIL_K", 2.0), getattr(config, "MAX_HOLD_BARS_15M", 48)
    if aln_ok:
        return "alignment", getattr(config, "ATR_TRAIL_K", 2.0), getattr(config, "MAX_HOLD_BARS_15M", 48)
    return None
...
async def _build_candidates_for_symbol(
    ...
    try:
        for i in range(max(25, 5), len(c) - 1):
            ...
            if picked is None:
                continue
            mode, trail_k, max_hold = picked
            candidate_score = _candidate_score(feat, i, c, mode)
            slope_arr = feat.get("ema20_slope", feat.get("slope"))
            slope = float(slope_arr[i]) if slope_arr is not None and np.isfinite(slope_arr[i]) else 0.0
            adx = float(feat["adx"][i]) if np.isfinite(feat["adx"][i]) else 0.0
            rsi = float(feat["rsi"][i]) if np.isfinite(feat["rsi"][i]) else 50.0
            ...
            if len(open_positions) >= effective_max_positions:
                ...
                for open_key, open_trade in open_positions.items():
                    ...
                    if extra_delta is None:
                        continue
                    if candidate.score < open_trade.entry_score + candidate_min_delta + extra_delta:
                        continue
                    replaceable.append((open_trade.entry_score, open_key, open_trade, idx, price))
