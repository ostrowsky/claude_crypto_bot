...
class TestPortfolioReplacement(unittest.TestCase):
    ...
    def test_T165H_replacement_disabled_by_default(self):
        import config
        from monitor import OpenPosition, MonitorState, _find_replaceable_position
        ...
        self.assertFalse(config.PORTFOLIO_REPLACE_ENABLED)
        self.assertIsNone(_find_replaceable_position(state, candidate_score=99.0, candidate_mode="breakout"))
...
class TestMLRankerPatches(unittest.TestCase):
    """T259-T268: Tests for ML ranker patches (target improvement, dow removal, acceptance gate)."""

    def test_T259_target_return_uses_min_worst_case(self):
        """PATCH: _target_return returns min(ret_3, exit_pnl, ret_5) not weighted blend."""
        import ml_candidate_ranker
        rec = {"labels": {"ret_3": -0.5, "ret_5": 1.0, "trade_taken": True, "trade_exit_pnl": -0.8}}
        result = ml_candidate_ranker._target_return(rec)
        self.assertAlmostEqual(result, -0.8, places=5)

    def test_T260_target_return_uses_min_ret3_ret5_without_exit(self):
        """PATCH: Without exit_pnl uses min(ret_3, ret_5)."""
        import ml_candidate_ranker
        rec = {"labels": {"ret_3": -0.3, "ret_5": 0.8, "trade_taken": False, "trade_exit_pnl": None}}
        result = ml_candidate_ranker._target_return(rec)
        self.assertAlmostEqual(result, -0.3, places=5)

    def test_T261_dataset_coverage_days_counts_unique_dates(self):
        """PATCH: dataset_coverage_days returns number of unique calendar days."""
        import ml_candidate_ranker
        rows = [
            {"ts_signal": "2026-03-24T10:00:00Z"},
            {"ts_signal": "2026-03-24T11:00:00Z"},
            {"ts_signal": "2026-03-25T10:00:00Z"},
            {"ts_signal": "2026-03-26T10:00:00Z"},
        ]
        days = ml_candidate_ranker.dataset_coverage_days(rows)
        self.assertEqual(days, 3.0)

    def test_T262_temporal_features_excluded_when_less_than_60_days(self):
        """PATCH: dow_sin and dow_cos removed from features when coverage < 60 days."""
        import ml_candidate_ranker
        features_short = ml_candidate_ranker._temporal_feature_names(days_covered=6.0)
        features_long  = ml_candidate_ranker._temporal_feature_names(days_covered=90.0)
        self.assertNotIn("dow_sin", features_short)
        self.assertNotIn("dow_cos", features_short)
        self.assertIn("dow_sin", features_long)
        self.assertIn("dow_cos", features_long)

    def test_T263_acceptance_gate_fails_on_low_auc(self):
        """PATCH: Acceptance gate rejects if AUC < 0.55."""
        import rl_headless_worker
        result = {"top1_delta": 0.20, "train_report": {"validation": {"mlp": {"auc": 0.48}}}}
        passed, reason = rl_headless_worker._check_acceptance_gate(result, live_rows=2000)
        self.assertFalse(passed)
        self.assertIn("AUC", reason)

    def test_T264_acceptance_gate_fails_on_insufficient_live_rows(self):
        """PATCH: Acceptance gate rejects if live_rows < 1000 even if AUC is good."""
        import rl_headless_worker
        result = {"top1_delta": 0.20, "train_report": {"validation": {"mlp": {"auc": 0.57}}}}
        passed, reason = rl_headless_worker._check_acceptance_gate(result, live_rows=500)
        self.assertFalse(passed)
        self.assertIn("live_rows", reason)

    def test_T265_acceptance_gate_passes_when_all_criteria_met(self):
        """PATCH: Acceptance gate passes when AUC >= 0.55, delta >= 0.15%, live_rows >= 1000."""
        import rl_headless_worker
        result = {"top1_delta": 0.20, "train_report": {"validation": {"mlp": {"auc": 0.57}}}}
        passed, reason = rl_headless_worker._check_acceptance_gate(result, live_rows=1500)
        self.assertTrue(passed)
        self.assertIn("AUC", reason)

    def test_T266_rl_config_override_file_written_on_gate_pass(self):
        """PATCH: _apply_config_override writes rl_config_overrides.json."""
        import rl_headless_worker, json, tempfile
        from pathlib import Path
        from unittest.mock import patch
        with tempfile.TemporaryDirectory() as td:
            override_path = Path(td) / "rl_config_overrides.json"
            with patch.object(rl_headless_worker, "RL_CONFIG_OVERRIDE_FILE", override_path):
                rl_headless_worker._apply_config_override(
                    "ML_CANDIDATE_RANKER_SCORE_WEIGHT", 5.0, "test reason"
                )
                self.assertTrue(override_path.exists())
                data = json.loads(override_path.read_text())
                self.assertEqual(data["ML_CANDIDATE_RANKER_SCORE_WEIGHT"], 5.0)
                self.assertEqual(data["_reason"], "test reason")

    def test_T267_shadow_report_includes_live_and_bootstrap_rows(self):
        """PATCH: shadow report source contains live_rows and bootstrap_rows."""
        src = open("/home/claude/report_candidate_ranker_shadow.py", "rb").read()
        self.assertIn(b"live_rows", src)
        self.assertIn(b"bootstrap_rows", src)

    def test_T268_monitor_apply_rl_config_overrides_wired(self):
        """PATCH: monitor.py contains _apply_rl_config_overrides and rl_config_overrides.json reference."""
        monitor_src = open("/home/claude/monitor.py", "rb").read()
        self.assertIn(b"_apply_rl_config_overrides", monitor_src)
        self.assertIn(b"rl_config_overrides.json", monitor_src)
        self.assertIn(b"_RL_OVERRIDES_LOADED", monitor_src)
...
class TestRestartMonitoringGuards(unittest.TestCase):
    ...
    def test_T215_post_init_runs_startup_scan_in_background(self):
        ...
        self.assertIn("Skipping startup market_scan because restored positions are already being monitored", block)
...
class TestTopMoverPrioritizationAndAudit(unittest.TestCase):
    def test_T242_positions_are_sorted_by_forecast_return(self):
        bot_src = Path("bot.py").read_text(encoding="utf-8")
        self.assertIn("for sym, pos in sorted(state.positions.items(), key=_sort_key, reverse=True):", bot_src)
        self.assertIn('getattr(coin_report, "forecast_return_pct"', bot_src)
        self.assertIn('getattr(coin_report, "today_change_pct"', bot_src)
