...
С 2026-08-14 метрика публикуется вторым числом — на неизменяемых later-EOD
метках из биржевых klines (`NS_IMMUTABLE_LABELS_ENABLED`,
`files/immutable_labels.py`). Именно **рядом**, а не вместо: у двух чисел разные
знаменатели (глобальный top-20 ∩ вотчлист против top-20 внутри вотчлиста),
поэтому их разность измеряет знаменатель, а не бота — в JSON стоит
`immutable_comparable_to_primary = False`. Сравнение по одному правилу лежит в
`files/_backtest_immutable_ns.py`: за 117 дней составы победителей расходятся на
38% пар «день-монета», а EC двигается 0.0243 → 0.0236. Изменилось происхождение
числа, а не число. «Предварительный» снимется, когда на снапшотную метку
перестанет опираться и `train_top_gainer` (`TRAIN_IMMUTABLE_LABELS_ENABLED`,
сейчас **False**).
...
## Харнесс достоверности — как не обманывать себя

Каждое число в этом проекте когда-то выглядело хорошо и ничего не значило.
Правила ниже написаны по фактам нарушений, каждое стоило времени.
**Изменение не считается законченным, пока на них не ответили.**

1. **Доля без базовой ставки — не доказательство.** «recall@20 = 100%» месяцами
   считался достижением; бандит достигает его тем, что говорит ENTER **73% всего
   подряд** — лифт 1.36. Рядом с любой долей публиковать базу и лифт.
2. **Называть признаки, которые уже содержат ответ.** У `top_gainer_model`
   AUC 0.99, при этом на входе `tg_return_since_open`, а метка — «дневной рост
   попал в топ-20». Такая модель подтверждает уже идущее движение, а не
   предсказывает его.
3. **In-sample числа — не достижение.** Только holdout и только разбиение
   ПО ВРЕМЕНИ (строки одного дня протекают друг в друга).
4. **Сравнивать только сопоставимые окна.** 5 рабочих дней против 10 дали ложное
   «стало хуже» после простоя. Иначе честный ответ — «рано судить».
5. **Метрика обязана знать, чего она не знает.** До 2026-08-05 дни простоя
   считались промахами — сбой читался как деградация.
6. **Гейт проверять на СОБСТВЕННЫХ входах бота, а не на рынке.** Фильтр слабого
   отрыва работал на рыночных эпизодах и оказался бесполезен на входах бота:
   верхние гейты не пускают его в эту популяцию.
7. **Изменения поведения — за флагом, с откатом, сначала в shadow**, если shadow
   способен ответить на вопрос.
8. **Отрицательные результаты коммитить** вместе с числами, которые их убили.
...
### Наблюдаемый набор — ВЕСЬ watchlist (2026-08-12)

`MONITOR_FULL_WATCHLIST = True`: опрашиваются все монеты списка, поэтому «монета
не была в наблюдении» больше не может быть причиной пропуска.

Раньше набор был выборочным, и `bot.py::_update_hot_coins` каждые 30 минут
(auto-reanalyze) **полностью перезаписывал** `state.hot_coins`, стирая все
промоуты (discovery / decoupling / soft). Наблюдалось: 18 монет → 9 за 11 минут,
POL не сканировался 15 дней. Скан и так анализирует весь watchlist
(`in_play + skipped` = все монеты), поэтому полный охват не стоит ни одного
лишнего запроса; символы, потерянные сканом из-за разового сбоя загрузки,
добираются заглушкой `CoinReport` (свечи `_poll_coin` грузит сам).

Стоимость ограничена РОТАЦИЕЙ, а не размером набора: `MAX_POLL_PER_CYCLE = 45`
монет за 60-секундный цикл → полный обход ~105 монет за ~3 цикла (~3 минуты).
**Монеты с открытой позицией опрашиваются каждый цикл** независимо от ротации —
трейлинг-стоп ждать не может. Откат: `MONITOR_FULL_WATCHLIST = False`.

Отключено как ставшее лишним: `SOFT_PROMOTE_ENABLED=False`, промоут через
decoupling (сам расчёт декоплинга и shadow-логирование сохранены).
...
### Зависание event loop + самоубийство бота (2026-08-04)

Две наложившиеся ошибки: бот тихо умирал и не поднимался (23.07 — 8 дней без
сигналов; 04.08 — 7 часов, обнаружено по таймауту /menu).

1. Проставка меток (`_fill_trade_outcome_labels`, `_fill_forward_labels` в
   monitor.py) вызывала полную перезапись датасетов (critic ~128 МБ, ml ~114 МБ)
   прямо в asyncio-цикле → заморозки на 130-300 с, таймауты Telegram-интерфейса,
   переподключения WS. Исправлено: запись вынесена в отдельный поток
   (`_LABEL_EXECUTOR`, один воркер — перезаписи не идут параллельно).
   Замер: заморозки каждые несколько минут → 0 предупреждений о лаге.
   Осталось: перестать переписывать файлы целиком (append-only + компакция).

2. UI-watchdog при зависании делал `os._exit(2)` в расчёте на wrapper, которого
   нет (`bot_bg_runner.cmd` запускает python один раз). Исправлено:
   `UI_WATCHDOG_FORCE_EXIT_AFTER_WARNS 3 → 0` + защита `> 0` (иначе 0 означал бы
   выход по ПЕРВОМУ предупреждению). Включать только вместе с wrapper'ом.
...
«Стоило входить» = строка попала в top-`BANDIT_FORWARD_TOP_N` своего дня по
движению, которое **остаётся впереди после снимка**, и это движение превышает
`BANDIT_FORWARD_MIN_PCT`. Раньше условием было «монета в top-20 по доходности
дня» — см. «Утечка в обучении бандита» ниже.

| Ситуация | Награда |
|----------|---------|
| ENTER + стоило входить | +1.0 |
| SKIP + стоило входить | -0.8 (штраф за пропуск!) |
| SKIP + ловить нечего | +0.10 |
| ENTER + ловить нечего | -0.12 |

### Утечка в обучении бандита (исправлена 2026-08-13) — TH-01/TH-03/TH-04

Награду платили за `label_top20`, а `use_earliest_snapshot=True` берёт самую
раннюю запись календарного дня — то есть снимок 00 UTC, EOD-разрешение уже
**закрытого** дня (41% обучающих строк). У 34.8% строк
`|eod_return − return_since_open| < 0.5%` — решать нечего; у строк, за которые
платили +1.0, впереди оставалась медиана **+1.75%**. Свежий бандит на этой метке
даёт **лифт 0.65×** на временном holdout — хуже случайного выбора. Отсюда и
бралось «recall@20 = 100%».

Вместе с меткой чинились два дефекта того же тракта:
- `_load_top_gainer_dataset` читал **первые** 50k строк из 118 625 — окно
  обучения было заморожено на 2026-06-05 на 69 дней. Теперь читается хвост.
- `train_entry_bandit` каждый запуск дообучался поверх сохранённого состояния:
  8.39M обновлений из ~44.6k уникальных примеров (~188-кратное переваривание).
  Это вшивало старую метку в `A/b` и раздувало уверенность без новой информации —
  поэтому «рост UCB-разделения» тоже ничего не измерял. Теперь состояние
  пересобирается с нуля.

Сравнение разметок (309 дней, сплит по времени, holdout 9765 строк; цель — у
монеты остаётся ≥ +3% хода, базовая ставка 2.0%), скрипт
`files/_backtest_bandit_leak_fix.py`:

| разметка обучения | ENTER | поймано | точность | лифт |
|---|---|---|---|---|
| старая `label_top20` | 80.3% | 52% | 1.3% | **0.65×** |
| ранг top-20, без пола | 98.0% | 100% | 2.1% | 1.02× |
| ранг top-20 + пол +3% | 43.5% | 99% | 4.6% | 2.28× |
| **ранг top-10 + пол +3%** | **24.3%** | **99%** | **8.3%** | **4.07×** |
| пол +3% без ранга | 80.0% | 100% | 2.5% | 1.25× |

Пол обязателен: чистый ранг штампует ровно N победителей в день независимо от
рынка и скатывается к «ENTER на всё» (1.02×).

Выкачено: метка + пересборка + хвост окна; состояние пересобрано (37 144
обновления, окно 2026-06-23..08-13, база позитивов 7.5%, отброшено 6 289
уже-решённых строк). Старое состояние сохранено в
`files/bandit_entry_state.pre_leakfix.json`.
Откат = `BANDIT_FORWARD_REWARD_ENABLED=False` + вернуть этот файл.

Оговорка (§0a правило 6): измерено на популяции датасета — все монеты вотчлиста
на дневном снимке. Живьём бандит судит кандидатов, уже прошедших верхние
фильтры, поэтому живой эффект читается по логам решений, а не предполагается.

### Источники обучения (train_entry_bandit)

1. **Primary:** `top_gainer_dataset.jsonl` — ВСЕ ~105 монет watchlist × N снимков
   в день. Читается ХВОСТ (`BANDIT_TG_MAX_RECORDS`), разметка — общий хелпер
   `offline_rl._build_universal_rows`, тот же, что использует оценка, чтобы
   обучение и проверка не разъезжались.
2. **Secondary:** `critic_dataset.jsonl` — сигналы бота с реальными результатами.
   Размечается рангом `ret_10` внутри дня (8.4% записей позитивны); у ранговой
   метки та же слабость фиксированной базы, поэтому этот источник — следующий
   на пересмотр.
...
**AUC ~0.99 — это утечка, а не качество.** Тир-метки пишутся из того же
rolling-24h снимка, что и признаки, поэтому `tg_return_since_open` одновременно
вход и почти ответ (TH-03). На неизменяемых later-EOD метках те же признаки дают
**0.83** — честный уровень (`files/_backtest_immutable_training_labels.py`,
120 088 строк, окно 2025-10-07..2026-08-14).

Неизменяемый путь выкачен под флагом `TRAIN_IMMUTABLE_LABELS_ENABLED` (**False**)
с порогом `TRAIN_IMMUTABLE_LABEL_MIN_PCT = 5.0`. **Не включать, не решив сначала
судьбу тиров:** хранилище меток содержит только вотчлист, поэтому тир становится
«top-N внутри вотчлиста», и под полом +5% `top20` и `top50` на holdout
**побайтово совпадают** (`top10` отличается на 0.05% строк). Лестница в
`top_gainer_model.py` стоит на фиксированных порогах 0.3/0.3/0.35/0.4,
откалиброванных под базовые ставки 1.49/3.15/6.31/15.34% — четыре одинаковые
головы с базой ~3% ломают все ступени разом. Варианты и доказательства:
`docs/specs/features/top-gainer-immutable-training-labels-spec.md`.
...
**Вся таблица выше получена на протекшей метке `label_top20` и на повторно
переваренных строках — 2026-08-13 она закрыта как историческая.** Recall 99–100%,
рост UCB-разделения и AUC 0.99 измеряли прошлое, а не предсказание. Сравнивать их
с числами после исправления нельзя (§0a правило 4: несопоставимые окна).

**Что осталось верным из ключевых событий:**
- **Apr 15**: `bandit_n_signal` упёрся в cap=8000. Фикс: `BANDIT_CRITIC_MAX_RECORDS=25_000`.

**Новая база отсчёта (2026-08-13, честный временной holdout —
`evaluate_bandit_accuracy` обучает свежий бандит на ранних днях и судит по
последним 7):**

| Дата | scope | recall | доля ENTER | база | лифт |
|------|-------|--------|-----------|------|------|
| Aug 13 | out-of-sample, 45 дней train / 7 eval | 74.2% | 35.3% | 8.1% | **2.10×** |
| Aug 13 | in-sample эхо (тот же живой бандит) | 85.5% | 57.8% | 8.1% | 1.48× |

Обе строки печатаются рядом намеренно: in-sample эхо выше по recall и хуже по
лифту — ровно тот разрыв, который §0 правило 3 запрещает схлопывать в одно число.
