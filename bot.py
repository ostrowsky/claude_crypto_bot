...
async def _reply_text_with_retry(msg, text: str, retries: int = 2, **kwargs):
    for attempt in range(retries + 1):
        try:
...
def _positions_message_html() -> str:
    ...
    shown = 0
    for sym, pos in sorted(state.positions.items(), key=_sort_key, reverse=True):
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── 📊 Активные сигналы ───────────────────────────────────────────────────
        ...
        await query.edit_message_text(_positions_message_html(), parse_mode=ParseMode.HTML)
        return
        _refresh_positions_state()
        if not state.positions:
            max_pos = getattr(config, "MAX_OPEN_POSITIONS", 6)
...
async def _post_init(app: Application) -> None:
    ...
    if state.positions:
        log.info(
            "Skipping startup market_scan because restored positions are already being monitored"
        )
    else:
