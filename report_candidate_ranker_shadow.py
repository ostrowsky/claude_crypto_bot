...
def _bar_group_key(rec: dict) -> str:
    return f"{rec.get('ts_signal','')}|{rec.get('tf','')}"
...
def build_shadow_report(
    ...
    *,
    ...
    for rec in rows:
        rec["_pred_proba"] = ml_candidate_ranker.predict_proba_from_candidate_payload(payload, rec)
    ...
    for group in grouped_rows:
        ...
        ranker = sorted(group, key=lambda r: (r.get("_pred_proba", 0.0), r.get("_candidate_score", 0.0)), reverse=True)[0]
    ...
    # PATCH: count live (non-bootstrap) rows for acceptance gate display
    live_rows = sum(
        1 for r in rows
        if str((r.get("decision") or {}).get("reason_code", "")) != "bootstrap_ml_dataset"
    )
    bootstrap_rows = len(rows) - live_rows
    return {
...
def render_text(report: Dict[str, Any]) -> str:
    live = report.get("live_rows", 0)
    bootstrap = report.get("bootstrap_rows", report["rows_total"] - live)
    live_pct = 100 * live / max(1, report["rows_total"])
    lines = [
        "Candidate Ranker Shadow Report",
        f"Rows: {report['rows_total']} (live={live} [{live_pct:.0f}%], bootstrap={bootstrap})",
