...
from contextual_bandit import feedback_entry as bandit_feedback
    ...
    async def on_trade_closed(
        ...
        *,
        ...
        if bandit_arm is not None:
            exp.bandit_arm = bandit_arm
            exp.bandit_info = bandit_info
        ...
        # Feed bandit with trade outcome
        if exp.reward is not None and exp.state:
            try:
                bandit_feedback(
                    state=exp.state,
                    arm=exp.bandit_arm if exp.bandit_arm is not None else 2,
                    reward=exp.reward,
                    mode=exp.mode,
                    tf=exp.tf,
                ...
                log.warning("Bandit feedback failed: %s", e)

        # Trigger offline RL training if enough new trades
        if self._trades_since_optimize >= OPTIMIZE_EVERY_N_TRADES:
            asyncio.create_task(self._run_offline_training_async())
            self._trades_since_optimize = 0

    async def _run_offline_training_async(self) -> None:
        """Run full offline RL training (bandit + exit + CMA-ES) in thread pool."""
        try:
            log.info("RL: starting offline training cycle...")
            results = await asyncio.to_thread(
                run_offline_training,
                batch_size=256,
                min_new_trades=5,
            )
            # Apply CMA-ES best params if available
            from rl_optimizer import load_optimizer_state
            state = load_optimizer_state()
            if state.best_params:
                self._best_params.update(state.best_params)
                RL_PARAMS_FILE.write_text(json.dumps(self._best_params, indent=2))
            log.info("RL: offline training complete: %s", {
                k: v.get("status", "?") for k, v in results.items()
                if isinstance(v, dict)
            })
        except Exception as e:
            log.error("RL offline training failed: %s", e)
