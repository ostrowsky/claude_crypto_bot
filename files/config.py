...
# Validated: high-vol + market-decoupled watchlist coins are ~2x more likely
# to make a forward idiosyncratic big move (top-gainer rocket).
# Report: docs/reports/2026-05-07-decoupling-validation.md
# SHADOW only — logged on entry events for bandit shadow-replay; does NOT
# change any entry/exit decision. Rollback = DECOUPLING_SHADOW_ENABLED=False.
DECOUPLING_SHADOW_ENABLED: bool = True            # compute + log score (no behavior change)
DECOUPLING_TF: str = "1h"                         # trailing-corr timeframe
DECOUPLING_WINDOW_BARS: int = 168                 # ~7d on 1h (validated window)
DECOUPLING_VOL_PCTILE_MIN: float = 0.66           # flag floor: top-third vol
DECOUPLING_CORR_MAX: float = 0.60                 # flag ceiling: trailing corr to basket
