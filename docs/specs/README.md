...
| [`canonical-ex1-zigzag`](./features/canonical-ex1-zigzag-spec.md) | shipped 2026-08-17 | TH-02/TH-11: the daily aggregator passed NO arguments, so `--use-zigzag` could never be set and EX1 was always the proxy. Caller fixed; the metric is still `unknown` on purpose — canonical coverage is 36% of trades and **9 of 27** top-20 rows. First diagnosis (missing 15m history) was WRONG and is corrected in the spec: zero misses come from absent files — 17 of 18 are trades that overlap no detected uptrend at 1h. Publishing a half-proxy average would be two definitions under one name. |
