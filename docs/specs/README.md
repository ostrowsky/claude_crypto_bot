...
| [`continuous-improvement-agent`](./features/continuous-improvement-agent-spec.md) | design | LLM agent proposes hypotheses, an independent non-LLM validator scores them, judges and gates decide. Built around why the existing L1–L7 pipeline died (invented config keys, exact-key dedup, no validator, no meta-eval). |
