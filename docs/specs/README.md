...
| [`canonical-ex1-zigzag`](./features/canonical-ex1-zigzag-spec.md) | shipped 2026-08-17 | TH-02/TH-11: the daily aggregator passed NO arguments, so `--use-zigzag` could never be set and EX1 was always the proxy. Caller fixed; the metric is still `unknown` on purpose — canonical coverage is 36% of trades and **9 of 27** top-20 rows, because `history/` holds 854 files at 1h against 98 at 15m. Publishing a half-proxy average would be two definitions under one name. |
