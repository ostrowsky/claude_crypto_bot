...
def check_entry_conditions(
    ...
    # PATCH: global vol hard floor - blocks COTIUSDT-style ghost entries (vol_x=0.075).
    _vol_hard_floor = float(getattr(config, "ENTRY_VOL_HARD_FLOOR", 0.30))
    if np.isfinite(vx) and vx < _vol_hard_floor:
        return False, f"vol_x {vx:.3f} < global hard floor {_vol_hard_floor:.2f}"
    # Non-bull day vol floor: stricter minimum on all entries when BTC < EMA50
    _is_bull = bool(getattr(config, "_bull_day_active", False))
    if not _is_bull:
        _nonbull_vol = float(getattr(config, "ENTRY_VOL_FLOOR_NONBULL", 0.70))
        if np.isfinite(vx) and vx < _nonbull_vol:
            return False, f"vol_x {vx:.3f} < non-bull day floor {_nonbull_vol:.2f}"
    ...
    # ── Trend-режим: дополнительные строгие проверки ──────────────────────────
    # Применяется только к "trend" (не strong_trend, не impulse_speed).
    # По replay 7 дней: trend WR=22.2%, EV=−0.49% — базовые пороги слишком слабые.
    if getattr(config, "TREND_STRICT_ENABLED", True) and not early_1h_override:
        # Определяем будет ли этот вход классифицирован как "trend"
        # (т.е. НЕ strong_trend: ADX < STRONG_ADX_MIN или vol_x < STRONG_VOL_MIN)
        _would_be_trend = not (
            np.isfinite(adx) and adx >= config.STRONG_ADX_MIN and
            np.isfinite(vx) and vx >= config.STRONG_VOL_MIN
        )
        # И НЕ impulse_speed: нет быстрого ценового движения
        if _would_be_trend:
            c_arr = feat.get("close")
            if c_arr is not None and i >= 3:
                _price_speed = (float(c_arr[i]) - float(c_arr[i - 3])) / max(float(c_arr[i - 3]), 1e-12) * 100.0
                _would_be_trend = _price_speed < 1.5  # 1.5% за 3 бара = порог impulse_speed

        if _would_be_trend:
            # Slope должен быть выше TREND_SLOPE_MIN
            trend_slope_min = float(getattr(config, "TREND_SLOPE_MIN", 0.20))
            if slp < trend_slope_min:
                return False, f"trend slope {slp:+.2f}% < {trend_slope_min:.2f}% (trend strict)"

            # ADX должен быть выше TREND_ADX_MIN
            trend_adx_min = float(getattr(config, "TREND_ADX_MIN", 24.0))
            if adx < trend_adx_min:
                return False, f"trend ADX {adx:.1f} < {trend_adx_min:.1f} (trend strict)"

            # MACD должен расти N баров подряд — не только последний бар
            trend_macd_bars = int(getattr(config, "TREND_MACD_CONFIRM_BARS", 3))
            if trend_macd_bars > 1 and i >= trend_macd_bars:
                _macd = feat["macd_hist"]
                for _k in range(1, trend_macd_bars):
                    _m_cur  = _macd[i - _k + 1] if i - _k + 1 >= 0 else np.nan
                    _m_prev = _macd[i - _k]     if i - _k     >= 0 else np.nan
                    if not (np.isfinite(_m_cur) and np.isfinite(_m_prev) and _m_cur > _m_prev):
                        return False, (
                            f"trend MACD не растёт {trend_macd_bars} баров подряд "
                            f"(нужна устойчивость, не всплеск)"
                        )

    # Range max: динамический + режим
    _range_max = _get_effective_range_max(feat, i, regime)
    # FIX: hard cap for trend mode on non-bull day.
    # Dynamic range can expand threshold based on historical volatility,
    # but JASMY audit showed daily_range=8.6% entering as trend on non-bull day.
    _is_bull_day_now = bool(getattr(config, "_bull_day_active", False))
    if not _is_bull_day_now:
        _trend_hard_cap = float(getattr(config, "TREND_NONBULL_DAILY_RANGE_HARD_CAP", 8.0))
        _range_max = min(_range_max, _trend_hard_cap)
    ...
    *,
    ...
    if two_closes_below:
        if not mode_aware or mode in aggressive_modes or negative_slope or adx_weak_effective:
            # ── EMA50-защита для retest/breakout ─────────────────────────────
            # Пока цена выше EMA50 — pullback к EMA20 это нормальная коррекция,
            # НЕ признак разворота. Выходить только когда сломана EMA50.
            # Без этого бот выходил из прибыльных позиций на первом баре откатa.
            if mode_aware and mode in aggressive_modes:
                _protect_key = "RETEST_EMA50_PROTECT" if mode == "retest" else "BREAKOUT_EMA50_PROTECT"
                _ema50_protect = getattr(config, _protect_key, True)
                _min_bars_key = "BREAKOUT_MIN_BARS_EMA_EXIT" if mode == "breakout" else None
                _min_bars_exit = int(getattr(config, _min_bars_key, 0)) if _min_bars_key else 0
                if _ema50_protect:
                    _es_arr = feat.get("ema_slow")
                    if _es_arr is not None and i < len(_es_arr) and np.isfinite(float(_es_arr[i])):
                        _ema50_val = float(_es_arr[i])
                        if close > _ema50_val:
                            # Цена выше EMA50 — не выходим, даже если 2 закрытия под EMA20
                            pass
                        else:
                            return f"2 закрытия подряд ниже EMA20 ({ef:.6g}) — ранний разворот"
                    elif bars_elapsed >= _min_bars_exit:
                        return f"2 закрытия подряд ниже EMA20 ({ef:.6g}) — ранний разворот"
                elif bars_elapsed >= _min_bars_exit:
                    return f"2 закрытия подряд ниже EMA20 ({ef:.6g}) — ранний разворот"
            else:
    ...
    if below_ema20:
        if not mode_aware:
            ...
            # aggressive modes: дополнительно проверяем EMA50-защиту
            if negative_slope or adx_weak_effective:
                if mode in aggressive_modes:
                    _protect_key = "RETEST_EMA50_PROTECT" if mode == "retest" else "BREAKOUT_EMA50_PROTECT"
                    _ema50_protect = getattr(config, _protect_key, True)
                    if _ema50_protect:
                        _es_arr = feat.get("ema_slow")
                        if _es_arr is not None and i < len(_es_arr) and np.isfinite(float(_es_arr[i])):
                            if close > float(_es_arr[i]):
                                pass  # выше EMA50 — не выходим
                            else:
                                return f"цена ниже EMA20 ({ef:.6g}) + подтверждённая слабость"
                        else:
                            return f"цена ниже EMA20 ({ef:.6g}) + подтверждённая слабость"
...
def get_entry_mode(feat: Dict, i: int) -> str:
    ...
    if price_speed >= 1.5:
        # PATCH 29.03: impulse_speed needs a minimum trend context (ADX >= threshold).
        # ARUSDT ADX=15, CHZUSDT ADX=17 produced false breakout candles at 06 UTC.
        _imp_adx_min = float(getattr(config, "IMPULSE_SPEED_ADX_MIN", 18.0))
        if np.isfinite(adx) and adx >= _imp_adx_min:
            return "impulse_speed"
        # ADX too weak - use trend mode with standard stop, not wide impulse_speed stop
        return "trend"
    return "trend"
...
def check_retest_conditions(feat: Dict, i: int) -> Tuple[bool, str]:
    ...
    # PATCH 29.03 v2: hard cap on daily_range for retest.
    # WLDUSDT 1h retest with daily_range=14.34% should have been blocked.
    _retest_dr = float(dr_arr[i]) if np.isfinite(float(dr_arr[i])) else 0.0
    _retest_dr_cap = float(getattr(config, "RETEST_DAILY_RANGE_HARD_CAP", 10.0))
    if _retest_dr > 0 and _retest_dr > _retest_dr_cap:
        return False, f"retest daily_range {_retest_dr:.1f}% > hard cap {_retest_dr_cap:.1f}%"
    return True, ""
...
def check_alignment_conditions(feat: Dict, i: int) -> Tuple[bool, str]:
    """
...
async def _run_analysis(
    ...
    async with aiohttp.ClientSession() as session:
        # PATCH: parallel fetch + CPU in thread pool
        # asyncio.gather(all fetches) gives ~5-10x speedup vs sequential await task
        # analyze_coin (CPU: compute_features) moved to thread pool to not block event loop
        fetch_pairs = [
            (sym, tf)
            for sym in symbols
        ...
        fetch_results = await asyncio.gather(
            *[fetch_klines(session, sym, tf) for sym, tf in fetch_pairs],
            return_exceptions=True,
        )
        valid_inputs = [
            (sym, tf, data)
            for (sym, tf), data in zip(fetch_pairs, fetch_results)
            if data is not None and not isinstance(data, Exception)
        ]

        def _safe_analyze(sym: str, tf: str, data) -> "Optional[CoinReport]":
            try:
                return analyze_coin(sym, tf, data, from_scan=from_scan)
            except Exception as exc:
                import logging as _lg
                _lg.getLogger(__name__).warning("analyze_coin %s [%s]: %s", sym, tf, exc)
                return None

        analysis_results = await asyncio.gather(
            *[
                asyncio.to_thread(_safe_analyze, sym, tf, data)
                for sym, tf, data in valid_inputs
            ],
            return_exceptions=True,
        )
